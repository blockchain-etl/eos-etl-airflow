...
def build_export_dag(
    ...
    export_blocks_operator = add_export_task(
        True,
        "export_blocks",
        add_provider_uri_fallback_loop(export_blocks_command, provider_uris, provider_uris_shuffle),
    )

    return dag


def add_provider_uri_fallback_loop(python_callable, provider_uris, provider_uris_shuffle=False):
    """Tries each provider uri in provider_uris until the command succeeds"""
    if provider_uris_shuffle:
        provider_uris = list(provider_uris)
        random.shuffle(provider_uris)
