...
def build_load_dag(
    ...
    def add_load_tasks(task, file_format, time_partitioning_field='block_timestamp'):
        ...
        def enrich_task(ds, **kwargs):
            ...
            if load_all_partitions:
                for dest_project, dest_dataset_name in all_destination_projects:
                    ...
                    copy_job = client.copy_table(temp_table_ref, dest_table_ref, location='US',
    ...
    load_blocks_task = add_load_tasks('blocks', file_format='json', time_partitioning_field='timestamp')
    load_transactions_task = add_load_tasks('transactions', 'json')
    load_actions_task = add_load_tasks('actions', 'json')
    ...
    enrich_actions_task = add_enrich_tasks(
        'actions', dependencies=[load_actions_task])
    ...
    if notification_emails and len(notification_emails) > 0:
        send_email_task = EmailOperator(
            ...
            subject='EOS ETL Airflow Load DAG Succeeded',
            html_content='EOS ETL Airflow Load DAG Succeeded - {}'.format(chain),
            dag=dag
        )
